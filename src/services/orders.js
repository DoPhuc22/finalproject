...
export const cancelOrderOrUpdateReceiver = async (id, updateData) => {
  try {
    ...
    const response = await api.put(ORDER_ENDPOINTS.CANCEL_OR_UPDATE(id), payload);
    ...
    const updatedData = storedData.map(order => {
      if ((order.orderId || order.id) == id) {
        ...
        if (updateData.cancel === true) {
          ...
          if (payload.receiverName) updatedOrder.receiverName = payload.receiverName;
          if (payload.receiverPhone) updatedOrder.receiverPhone = payload.receiverPhone;
          if (payload.receiverAddress) updatedOrder.shippingAddress = payload.receiverAddress;
        }
    ...
    const updatedData = storedData.map(order => {
      if ((order.orderId || order.id) == id) {
        ...
        if (updateData.cancel === true) {
          ...
          if (updateData.receiverName) updatedOrder.receiverName = updateData.receiverName;
          if (updateData.receiverPhone) updatedOrder.receiverPhone = updateData.receiverPhone;
          if (updateData.receiverAddress) updatedOrder.shippingAddress = updateData.receiverAddress;
        }
    ...
    const updatedOrder = updatedData.find(order => (order.orderId || order.id) == id);
...
export const getUserOrders = async (userId) => {
  try {
    ...
    // Fallback to localStorage
    const storedData = getOrdersFromStorage();
    const userOrders = storedData.filter(order => 
...
export const createOrder = async (orderData) => {
  try {
    ...
    const requiredFields = ['userId', 'details', 'subtotal', 'total', 'paymentMethod', 'shippingAddress', 'receiverName', 'receiverPhone'];
    ...
    if (error.response) {
      ...
      switch (status) {
        case 400:
          errorMessage += "Thông tin đơn hàng không hợp lệ: " + (data?.message || "Dữ liệu không đúng định dạng");
          break;
        case 401:
...
export const getOrderById = async (id) => {
  try {
    ...
    // Fallback to localStorage
    const storedData = getOrdersFromStorage();
    const found = storedData.find(order => 
...
export const updateOrderStatus = async (id, status) => {
  try {
    ...
    // Đảm bảo status được gửi trong đúng định dạng
    const payload = { status: status };
    
    // Sử dụng PATCH thay vì PUT như đã đề cập trong yêu cầu
    const response = await api.patch(ORDER_ENDPOINTS.UPDATE_STATUS(id), payload);
    ...
    if (error.response) {
      ...
      switch (status) {
        ...
        case 500:
          ...
          break;
        default:
          errorMessage += data?.message ? ` ${data.message}` : ` Mã lỗi: ${status}`;
      }
    }
    ...
    enhancedError.newStatus = status;
...
export const deleteOrder = async (id) => {
  try {
    ...
    const storedData = getOrdersFromStorage();
    const filteredData = storedData.filter(order => 
      order.orderId !== id && order.id !== id
    );
