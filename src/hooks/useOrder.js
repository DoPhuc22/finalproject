...
const updateStatus = async (orderId, newStatus) => {
  try {
    setLoading(true);
    console.log("useOrder - Updating order status:", { orderId, newStatus });
    ...
    console.log("useOrder - Final status value being sent:", statusValue);
    console.log("useOrder - Type of status value:", typeof statusValue);
    ...
    setOrders((prev) => {
      const updatedList = prev.map((order) => {
        if (order.orderId === orderId) {
          return {
            ...order,
            status: statusValue,  // Lưu string đơn giản
